    ...
    <div class="py-8">
        <div class="max-w-4xl mx-auto">
            <div class="bg-slate-900 border border-slate-800 shadow-2xl rounded-2xl overflow-hidden relative">
                <!-- Background Glow -->
                <div
                    class="absolute top-0 right-0 -mt-10 -mr-10 w-40 h-40 bg-indigo-500/10 rounded-full blur-3xl pointer-events-none">
                </div>
                ...
                <div class="p-8 text-slate-300 relative z-10">
                    ...
                    <form action="{{ route('projects.store') }}" method="POST" x-data="projectForm()">
                        ...
                        <!-- 1. Nama Project -->
                        <div class="mb-6">
                            <label class="block text-sm font-bold text-white mb-2">Nama Project <span
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
                            <div>
                                <label class="block text-sm font-bold text-white mb-2">Tanggal Mulai <span
                                        class="text-red-500">*</span></label>
                                <input type="date" name="start_date"
                                    class="w-full rounded-2xl border-slate-700 bg-slate-950/50 text-white shadow-inner focus:border-indigo-500 focus:ring-indigo-500 focus:ring-2 focus:bg-slate-900 transition-all duration-300 p-3"
                                    required>
                            </div>
                            <div>
                                <label class="block text-sm font-bold text-white mb-2">Target Selesai <span
                                        class="text-red-500">*</span></label>
                        ...
                        <div class="mb-8" x-data="{
                            ...
                            initRole(id) {
                                // Jangan gunakan fallback 'Member', tetap simpan nilai yang diinputkan
                                return this.selectedRoles[id] !== undefined ? this.selectedRoles[id] : '';
                            },
                            ...
                            <label
                                class="block text-sm font-bold text-indigo-400 mb-4 uppercase tracking-wider flex items-center justify-between">
                                <span>Atur Tim & Posisi</span>
                                <span class="text-xs bg-indigo-500/20 text-indigo-300 px-2 py-1 rounded-full"
                                    x-text="selectedIds.length + ' Orang Dipilih'"></span>
                            </label>

                            <!-- Trigger Area (Tinggi 150px biar lega) -->
                            <div class="relative min-h-[150px] p-6 rounded-2xl bg-slate-950/30 border border-slate-800 flex flex-col items-center justify-center text-center border-dashed hover:border-indigo-500/50 transition-all cursor-pointer group overflow-hidden"
                                ...
                                <div x-show="selectedIds.length === 0"
                                    ...
                                    <div
                                        class="w-14 h-14 rounded-full bg-slate-900 flex items-center justify-center group-hover:bg-indigo-600 text-slate-400 group-hover:text-white transition-colors mb-3">
                                    ...
                                    <p class="text-sm font-bold text-white group-hover:text-indigo-400">Klik untuk
                                        Kelola Tim</p>
                                    <p class="text-xs text-slate-500 mt-1">Cari dan tambahkan karyawan ke project ini
                                    </p>
                                </div>

                                <!-- TERISI (Avatar Melayang) -->
                                <div x-show="selectedIds.length > 0"
                                    class="w-full flex flex-wrap items-center justify-center gap-4 z-10"
                                    style="display: none;">
                                    <template x-for="(user, index) in selectedUsers" :key="user.id">
                                        <div class="flex flex-col items-center animate-float-avatar"
                                            :style="`animation-delay: ${index * 0.2}s`">
                                            <div
                                                class="w-12 h-12 rounded-full bg-gradient-to-br from-indigo-500 to-purple-600 flex items-center justify-center text-white font-bold text-sm shadow-lg border-2 border-slate-800">
                                                <span x-text="user.initial"></span>
                                            </div>
                                            <span
                                                class="text-[10px] text-slate-300 mt-2 bg-slate-900/80 px-2 py-0.5 rounded-full"
                                                x-text="user.name.split(' ')[0]"></span>
                                        </div>
                                    </template>
                                    <!-- Ikon Tambah Kecil -->
                                    <div
                                        class="w-10 h-10 rounded-full border-2 border-dashed border-slate-600 flex items-center justify-center text-slate-500 hover:text-white transition-colors">
                            ...
                            <div x-show="openModal" style="display: none;"
                                ...
                                <div
                                    class="relative bg-slate-900 border border-slate-700 rounded-2xl shadow-2xl w-full max-w-2xl overflow-hidden transform transition-all flex flex-col max-h-[80vh]">
                                    ...
                                    <div
                                        ...
                                        <button type="button" @click="openModal = false"
                                            class="text-slate-400 hover:text-white"><svg class="w-6 h-6" fill="none"
                                    ...
                                    <div class="p-4 bg-slate-900 border-b border-slate-700">
                                        <div class="relative">
                                            ...
                                            <input type="text" x-model="search"
                                                class="w-full pl-10 pr-4 py-2.5 rounded-xl border-slate-700 bg-slate-950 text-white placeholder-slate-500 focus:ring-indigo-500 focus:border-indigo-500"
                                    ...
                                    <div class="overflow-y-auto p-4 space-y-3 custom-scrollbar flex-1">
                                        @foreach ($karyawan as $k)
                                            <div class="flex items-center gap-4 p-3 rounded-xl border border-slate-800 hover:border-indigo-500/30 transition-colors bg-slate-950/50 group"
                                                ...
                                                <div class="w-1/2"
                                                    x-show="selectedIds.includes('{{ $k->id }}')" x-transition>
                                                    <input type="text" name="roles[{{ $k->id }}]"
                                                        x-model="selectedRoles['{{ $k->id }}']"
                                    ...
                                    <div class="p-4 border-t border-slate-700 bg-slate-800/50 text-right">
                                        <button type="button" @click="openModal = false"
                                            class="px-6 py-2 bg-indigo-600 hover:bg-indigo-500 text-white font-bold rounded-lg shadow-lg transition-all">Selesai
                                            Plotting</button>
                                    </div>
                                </div>
                            </div>

                            <!-- Hidden inputs untuk submit members (roles dikirim via form input dengan name) -->
