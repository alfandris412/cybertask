    ...
    <x-slot name="header">
        <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
            <h2 class="font-bold text-xl text-white leading-tight">
                {{ __('Edit Project: ') }} <span class="text-indigo-400">{{ $project->name }}</span>
            </h2>
    ...
    <div class="py-8">
        <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="bg-slate-900 border border-slate-800 shadow-2xl rounded-2xl overflow-hidden relative">
                ...
                <div class="p-6 md:p-8 text-slate-300 relative z-10">
                    ...
                    <form action="{{ route('projects.update', $project) }}" method="POST">
                        ...
                        <div class="mb-6">
                            ...
                            <input type="text" name="name" value="{{ old('name', $project->name) }}" class="w-full rounded-2xl border-slate-700 bg-slate-950/50 text-white placeholder-slate-500 shadow-inner focus:border-indigo-500 focus:ring-indigo-500 focus:ring-2 focus:bg-slate-900 transition-all duration-300 p-3" required>
                            @error('name')<span class="text-red-500 text-xs mt-1 block">{{ $message }}</span>@enderror
                        </div>

                        <div class="mb-6">
                            <label class="block text-sm font-bold text-white mb-2">Deskripsi Singkat</label>
                            <textarea name="description" rows="4" class="w-full rounded-2xl border-slate-700 bg-slate-950/50 text-white placeholder-slate-500 shadow-inner focus:border-indigo-500 focus:ring-indigo-500 focus:ring-2 focus:bg-slate-900 transition-all duration-300 p-3">{{ old('description', $project->description) }}</textarea>
                        </div>
                        ...
                        <div class="mb-6">
                            ...
                            <div class="relative">
                                ...
                                <input type="url" name="repository_link" value="{{ old('repository_link', $project->repository_link) }}" class="w-full pl-10 rounded-2xl border-slate-700 bg-slate-950/50 text-white placeholder-slate-500 shadow-inner focus:border-indigo-500 focus:ring-indigo-500 focus:ring-2 focus:bg-slate-900 transition-all duration-300 p-3" placeholder="https://github.com/username/project-repo">
                            </div>
                        </div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
                            <div>
                                ...
                                <input type="date" name="start_date" value="{{ old('start_date', $project->start_date) }}" class="w-full rounded-2xl border-slate-700 bg-slate-950/50 text-white shadow-inner focus:border-indigo-500 focus:ring-indigo-500 focus:ring-2 focus:bg-slate-900 transition-all duration-300 p-3" required>
                            </div>
                            <div>
                                <label class="block text-sm font-bold text-white mb-2">Target Selesai <span class="text-red-500">*</span></label>
                                <input type="date" name="end_date" value="{{ old('end_date', $project->end_date) }}" class="w-full rounded-2xl border-slate-700 bg-slate-950/50 text-white shadow-inner focus:border-indigo-500 focus:ring-indigo-500 focus:ring-2 focus:bg-slate-900 transition-all duration-300 p-3" required>
                            </div>
                        </div>
                        ...
                        <div class="mb-8" x-data="{ 
                            openModal: false, 
                            search: '', 
                            selectedIds: {{ json_encode($selectedMembers) }},
                            roles: {{ json_encode($project->members->pluck('pivot.role', 'id')) }},
                            allKaryawan: {{ json_encode($karyawan->map(fn($u) => ['id' => $u->id, 'name' => $u->name, 'email' => $u->email, 'initial' => substr($u->name, 0, 2)])) }},
                            ...
                            toggleSelection(id) {
                                id = String(id);
                                if (this.selectedIds.map(String).includes(id)) {
                            ...
                            <div class="relative min-h-[150px] p-6 rounded-2xl bg-slate-950/30 border border-slate-800 flex flex-col items-center justify-center text-center border-dashed hover:border-indigo-500/50 transition-all cursor-pointer group overflow-hidden" @click="openModal = true">
                                ...
                                <div x-show="selectedIds.length === 0" class="flex flex-col items-center animate-fade-in-up">
                                    ...
                                    <p class="text-sm font-bold text-white group-hover:text-indigo-400">Tambah Anggota</p>
                                    <p class="text-xs text-slate-500">Klik untuk memilih karyawan</p>
                                </div>
                                ...
                                <div x-show="selectedIds.length > 0" class="w-full flex flex-wrap items-center justify-center gap-4 z-10" style="display: none;">
                                    <template x-for="(user, index) in selectedUsers" :key="user.id">
                                        <div class="flex flex-col items-center animate-float-avatar" :style="`animation-delay: ${index * 0.2}s`">
                                            ...
                                            <span class="text-[9px] text-indigo-400 mt-0.5" x-text="roles[user.id] ? roles[user.id] : 'Member'"></span>
                                        </div>
                                    </template>
                                ...
                                <div x-show="selectedIds.length > 0" class="absolute inset-0 bg-indigo-500/5 pointer-events-none"></div>
                            </div>
                            ...
                            <div x-show="openModal" style="display: none;" class="fixed inset-0 z-50 flex items-center justify-center px-4 sm:px-6">
                                ...
                                <div class="relative bg-slate-900 border border-slate-700 rounded-2xl shadow-2xl w-full max-w-2xl overflow-hidden transform transition-all flex flex-col max-h-[80vh]">
                                    ...
                                    <div class="px-6 py-4 border-b border-slate-700 flex justify-between items-center bg-slate-800/50">
                                        <h3 class="text-lg font-bold text-white">Update Tim & Posisi</h3>
                                        ...
                                        <div class="flex items-center gap-4 p-3 rounded-xl border border-slate-800 hover:border-indigo-500/30 transition-colors bg-slate-950/50 group"
                                            ...
                                            <div class="flex items-center h-full">
                                                <input type="checkbox" name="members[]" value="{{ $k->id }}" 
                                                       ...
                                                       :checked="selectedIds.map(String).includes(String({{ $k->id }}))">
                                            </div>
                                            ...
                                            <div class="w-1/2" x-show="selectedIds.map(String).includes(String({{ $k->id }}))" x-transition>
                                                <input type="text" name="roles[{{ $k->id }}]" x-model="roles[{{ $k->id }}]"
                                    ...
                                    <div class="p-4 border-t border-slate-700 bg-slate-800/50 text-right">
                                        <button type="button" @click="openModal = false" class="px-6 py-2 bg-indigo-600 hover:bg-indigo-500 text-white font-bold rounded-lg shadow-lg">Simpan Perubahan</button>
                                    </div>
                                </div>
                            </div>
                        </div>
